...
# --- MODUŁ FORUM ---
try:
    from forum_module import execute_forum_actions, discover_roots, auto_load_forum_context, save_forum_memory, load_forum_memory, parse_forum_markers
    FORUM_ENABLED = True
except ImportError:
    FORUM_ENABLED = False
...
def ew_log_phone(op_name, numer, wynik, grupa=None, kurier_ustalony=False,
                 ...
                 kurier_przewoznik=None, jezyk=None):
...
def ew_apply_tel_forum_wynik(ai_text, grupa_sprawy=None, numer=None):
    ...
    if _zrobione:
        ...
        # zostały jeszcze jakieś nieocenione dla tego numeru?
        try:
            _reszta = ew_pending_do_oceny(numer) if numer else []
            _dzien_n = datetime.now(pytz.timezone('Europe/Warsaw')).strftime("%Y-%m-%d")
            _ref_n = db.collection(col("ew_phone_log")).document(_dzien_n) \
                       .collection("nieocenione").document(str(numer or "?"))
            if _reszta:
                _ref_n.set({"ile": len(_reszta), "ostatnio": firestore.SERVER_TIMESTAMP}, merge=True)
            else:
                _ref_n.delete()
        except Exception:
            pass
...
def ew_log_telefonista_calls(numer, forum_ctx, doc_id=None, grupa=None):
    """Wykrywa NOWE odpowiedzi telefonistek i ZAPISUJE telefon OD RAZU.

    Fakt rozmowy, GRUPA, OSOBA (z nawiasu przy autorze) i GODZINA pochodzą wprost z forum —
    są pewne, więc nie mogą zależeć od tego, czy model wypisze znacznik. Status startuje jako
    'do_oceny' i zostaje uzupełniony przez ew_apply_tel_forum_wynik.

    Dedupe po SYGNATURACH konkretnych wpisów (grupa|osoba|data godz) — stary licznik
    'tel_forum_zalogowane' jest ignorowany, więc nie blokuje spraw otwieranych wcześniej.
    """
    _aut = _wykryte_odp_telefonistow(forum_ctx)
    if not _aut:
        return 0
    # 🟥 Sprawa NIE musi pochodzić z puli — wsad wklejony ręcznie też się liczy.
    #    Gdy rekord sprawy istnieje, trzymamy na nim sygnatury (szybciej);
    #    gdy go nie ma, dedupe opiera się na sprawdzeniu samego logu telefonów.
    ...
    for _grupa_tel, _osoba_tel, _dfor, _gfor in _aut:
        _dzien = _dfor or _today
        _godz = _gfor or datetime.now(_tz).strftime("%H:%M")
        _sygn = f"{_grupa_tel}|{_osoba_tel}|{_dzien} {_godz}"
        if _sygn in _stare:
            # SAMONAPRAWA: sygnatura mówi „rozliczone", ale telefonu może NIE BYĆ w logu
            # (np. zapis padł, dane wyczyszczono, albo wcześniejsza wersja oznaczyła wpis
            # bez zapisania rozmowy). Wtedy traktujemy wpis jak nowy i zapisujemy telefon.
            _istnieje = False
            try:
                for _d in db.collection(col("ew_phone_log")).document(_dzien).collection("calls") \
                           .where("numer_zamowienia", "==", str(numer or "").strip()).limit(30).stream():
                    _v = _d.to_dict() or {}
                    if (_v.get("operator") == _grupa_tel and _v.get("godzina") == _godz
                            and (_v.get("osoba") or "") == (_osoba_tel or "")):
                        _istnieje = True
                        break
            except Exception:
                _istnieje = True          # brak odczytu → nie ryzykujemy duplikatu
            if _istnieje:
                continue
        # bez rekordu sprawy: sprawdź, czy taki sam telefon nie jest już w logu tego dnia
        if _ref is None:
            try:
                _juz = db.collection(col("ew_phone_log")).document(_dzien).collection("calls") \
                         .where("numer_zamowienia", "==", str(numer or "").strip()).limit(30).stream()
                if any((_d.to_dict() or {}).get("operator") == _grupa_tel
                       and (_d.to_dict() or {}).get("godzina") == _godz
                       and ((_d.to_dict() or {}).get("osoba") or "") == (_osoba_tel or "")
                       for _d in _juz):
                    continue
            except Exception:
                pass
        try:
            _res = db.collection(col("ew_phone_log")).document(_dzien).collection("calls").add({
                "numer_zamowienia": str(numer or "").strip(),
                "operator": _grupa_tel,                 # GRUPA telefonistek
                "osoba": _osoba_tel or "",              # KONKRETNA osoba z nawiasu
                "grupa": grupa or "?",
                "wynik": "do_oceny",                    # AI uzupełni jakość rozmowy
                "efektywny": False,
                "kurier_ustalony": False,
                "zrodlo": "telefonista_zewn",
                "jezyk": jezyk_sprawy(None, grupa),
                "pz": "",
                "data_str": _dzien,                     # REALNA data wpisu z forum
                "godzina": _godz,                       # REALNA godzina wpisu z forum
                "wykryte_dnia": _today,
                "created_at": firestore.SERVER_TIMESTAMP,
            })
    ...
    if _nowe_sygn:
        if _ref is not None:
            try:
                ...
                try:
                    _ref.set({"tel_forum_sygnatury": list(_stare) + _nowe_sygn}, merge=True)
                except Exception:
                    pass
        st.session_state["_tel_forum_pending"] = (st.session_state.get("_tel_forum_pending") or []) + _pending
        # bezpiecznik: te telefony czekają na ocenę treści
        try:
            db.collection(col("ew_phone_log")).document(_today).collection("nieocenione") \
              .document(str(numer or "?")).set({
                  "numer_zamowienia": str(numer or ""), "ile": len(_pending),
                  "grupy": [a[0] for a in _aut], "grupa_sprawy": grupa or "?",
                  "ostatnio": firestore.SERVER_TIMESTAMP,
              }, merge=True)
        except Exception:
            pass
    return _ile

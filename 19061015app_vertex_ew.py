...
def ew_log_completion(op_name, channel="standard", grupa=None):
    ...
    grupa:   DE/FR/UKPL (grupa CASE'a) — do trwałego licznika grupowego gz_{grupa}
             czytanego przez Wieżowiec (tabela wsad-per-dzień + kafelek zakresu dat).
    ...
    # Trwały licznik grupowy (parent doc) — wg grupy CASE'a
...
"""
    ...
    # === ZAKOŃCZENIE WSADU ODWROTNEGO (główny panel — tuż przy rozmowie) ===
    # Oparte na TRWAŁYM markerze ew_reverse_active → przycisk jest widoczny zawsze, gdy trwa
    # analiza odwrotna, także dla case'a spoza puli / wklejonego ręcznie (ew_current_case=None).
    ...
    # NAJPEWNIEJSZY sygnał: wybrany Tryb Startowy (WA/MAIL/FORUM) z panelu bocznego. Skoro
    # operator wybrał tryb odwrotny, KAŻDA aktywna rozmowa w tym trybie dostaje przycisk
    # zakończenia — niezależnie od ścieżki wejścia (Szukaj case / Czyste okno / ręczny wsad)
    # i niezależnie od tego, czy ew_current_case jest ustawiony.
    if not _rev_active:
    ...
    if _rev_active:
        _rt = _rev_active
        st.markdown("---")
        st.caption(f"📨 Tryb odwrotny **{_rt}**. Po decyzji AI z TAG-KOPERTĄ zakończ wsad — zostanie policzony "
                   f"na Twoje konto (kanał {_rt}), niezależnie czy case był z puli, czy wklejony ręcznie.")
        if st.button(f"✅ Zakończ wsad odwrotny ({_rt}) → następny", type="primary", key="rev_finish_main"):
            tag, pz = None, None
            ...
            if tag:
                ...
                # Case z puli (pierwsze zakończenie) → oznacz zakończony; spoza puli → tylko loguj.
                if _doc and not _np:
                    ew_complete_case(_doc, result_tag=tag, result_pz=pz)
                log_stats(op_name, st.session_state.get("current_start_pz"), pz, st.session_state.get("current_project_idx", 0))
                ew_log_completion(op_name, channel=_ch, grupa=_grp)
                # Czysta karta — bez auto-pobrania ze standardowej puli.
                ...
                st.session_state._autopilot_loaded = False
                st.session_state.ew_wsad_ready = ""
                st.success(f"✅ Wsad odwrotny ({_rt}) zakończony i policzony. Czysta karta — rozpocznij kolejny.")
                st.rerun()
            else:
                st.error("❌ Brak TAGu (TAG-KOPERTA) w odpowiedzi AI — najpierw poproś AI o decyzję z TAGiem, potem zakończ.")

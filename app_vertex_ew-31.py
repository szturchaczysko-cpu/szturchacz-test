...
def ew_get_fresh_case(grupa, op_name):
    """Pobierz case ŚWIEŻAK — pomija nocne przeliczenia, bierze tylko nieprzeliczone.
    
    Różnice vs ew_get_next_case:
    - Wymusza autopilot_status IN (null, "") — pomija 'calculated', 'forum_action', 'bumped'
    - Zwraca None jeśli brak nieprzeliczonych (operator dostaje komunikat i może użyć zwykłego)
    - Oznacza case processed_via='swiezak' żeby admin widział w podglądzie
    """
    skipped_ids = st.session_state.get("ew_skipped_ids", set())
    ...
    # Filtr: tylko nieprzeliczone
    doc = None
    for d in all_free:
        data = d.to_dict()
        ap_status = data.get("autopilot_status")
        if ap_status in (None, "", "bumped"):
            # Dodatkowo pomiń forum_action (czeka na uwolnienie)
            doc = d
            break
    ...
    data = doc.to_dict()
    data["_doc_id"] = doc.id
    data["_is_swiezak"] = True  # flag dla UI
    return data


def ew_count_fresh_available(grupa):
    """Licz nieprzeliczone casy w grupie (dla operatora — pokazać ile świeżaków jest)."""
    try:
        q = (db.collection(col("ew_cases"))
             .where("grupa", "==", grupa)
             .where("status", "==", "wolny")
    ...
    if not czyste_okno:
        ...
        if not show_current_case:
            if wybrany_tryb_kod in ("WA", "MAIL", "FORUM"):
                ...
                if avail > 0:
                    _fresh_count = ew_count_fresh_available(operator_grupa)
                    ...
                    with _col_fresh:
                        _fresh_label = f"✨ Pobierz świeżaka ({_fresh_count})" if _fresh_count > 0 else "✨ Pobierz świeżaka"
                        if st.button(_fresh_label, type="secondary", key="get_fresh_case", help="Pobierz nieprzeliczony case — zostanie przeliczony Twoim aktualnym promptem"):
                            if _fresh_count == 0:
                                st.warning("Brak nieprzeliczonych casów. Użyj 'Pobierz case' dla nocnego przeliczenia.")
                            else:

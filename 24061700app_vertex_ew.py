...
with st.sidebar:
    ...
    # ── 📞 Woreczek telefoniczny (moduł Telefony) — pobierz case → ląduje w panelu jak zwykła sprawa ──
    with st.expander("📞 Woreczek telefoniczny", expanded=False):
        st.caption(f"Sprawy z delegowanym telefonem (FORUM_TEL=czekam_wynik) — TYLKO Twoja grupa "
                   f"({operator_grupa}). Kliknij Pobierz case telefoniczny: wsad ląduje po prawej i "
                   f"prowadzisz sprawę jak każdą inną — forum, analiza, finalny tag (usuwa ją z woreczka).")
        try:
            # Filtr po jednym polu (auto-indeks), grupę filtrujemy w Pythonie → bez indeksu złożonego.
            _all_wor = list(db.collection(col("ew_cases")).where("telefon_do_wykonania", "==", True).limit(60).stream())
            _woreczek = [_d for _d in _all_wor if (_d.to_dict() or {}).get("grupa") == operator_grupa]
            ...
            for _wd in _woreczek:
                ...
                try:
                    if _wretry is not None and hasattr(_wretry, "strftime"):
                        _wretry_txt = _wretry.astimezone(pytz.timezone("Europe/Warsaw")).strftime("%d.%m %H:%M")
                except Exception:
                    _wretry_txt = ""
                st.markdown(
                    f"**📞 {_wnum}** · {_wgrp}" + (f" · {_wpz}" if _wpz else "")
                ...
                if st.button("📞 Pobierz case telefoniczny", key=f"_wor_pull_{_wd.id}", type="primary"):
                    # Ładujemy jako case ODWROTNY (FORUM) + _phone_mode → render checklisty i przycisków wyniku.
                    # Reszta dzieje się normalną ścieżką: wsad po prawej, forum przy analizie, finisz tagiem.
                    ...
                    for _k in ("ew_reverse_active", "ew_reverse_no_pool", "ew_reverse_doc_id",
                               "ew_reverse_grupa"):
                        st.session_state.pop(_k, None)
                    st.rerun()
                _wpow = st.selectbox(
                    "Powód usunięcia:",
                    ["załatwione inaczej", "błędne zlecenie", "duplikat", "inny"],
                    key=f"_wor_pow_{_wd.id}",
                )
                if st.button("🗑️ Usuń z woreczka", key=f"_wor_drop_{_wd.id}"):
                    try:
                        db.collection(col("ew_cases")).document(_wd.id).update({
                            ...
                            "telefon_usuniety_powod": _wpow,
                        })
                        # Trwały log usunięć (przeżywa codzienny reset wsadu) → Wieżowiec „kto usunął".
                        try:
                            _tzw = pytz.timezone("Europe/Warsaw")
                            _tdw = datetime.now(_tzw).strftime("%Y-%m-%d")
                            db.collection(col("ew_woreczek_log")).document(_tdw).collection("usuniete").add({
                                "numer_zamowienia": _wnum, "przez": op_name, "powod": _wpow,
                                "grupa": _wgrp, "godzina": datetime.now(_tzw).strftime("%H:%M"),
                                "created_at": firestore.SERVER_TIMESTAMP,
                            })
                        except Exception:
                            pass
                        st.success(f"Usunięto {_wnum} z woreczka ({_wpow}).")
                        st.rerun()
                    except Exception as _e:
                        st.error(f"Błąd: {_e}")
                st.markdown("---")
...
if not st.session_state.chat_started:
    ...
    operator_dzwoniacy = "TAK" if OPERATORS_TEL.get(op_name, False) else "NIE"
...
"""
                ...
                for model_id in models_to_try:
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            # (B) Wynik telefonu (DZWONIĄCY) → log + status woreczka. System (AI) zdecydował
                            #     o jakości przez TEL_WYNIK. konkret → wypada; reszta → odroczony +2h (zostaje/wpada).
                            try:
                                if _tel_wynik:
                                    _ccp = st.session_state.get("ew_current_case") or {}
                                    _ccp_doc = _ccp.get("_doc_id")
                                    _nr_phone = st.session_state.get("chat_nrzam") or _ccp.get("numer_zamowienia")
                                    if _nr_phone:
                                        # _kurier_match żyje tylko w bloku E2 (gdy była akcja forum) — licz lokalnie,
                                        # inaczej owocny telefon „świeży" bez wpisu na forum poleciałby NameError.
                                    ...
                                    if _ccp_doc and not _ccp.get("_reverse_no_pool"):
                                        if _tel_wynik == "konkret":
                                            db.collection(col("ew_cases")).document(_ccp_doc).update({
                                                "telefon_do_wykonania": False,
                                                "telefon_status": "zrobiony",
                                                "telefon_last_op": op_name,
                                            })
                                        else:
                                            _retry = datetime.now(tz_pl) + timedelta(hours=2)
                                            db.collection(col("ew_cases")).document(_ccp_doc).update({
                                                "telefon_do_wykonania": True,
                                                "telefon_status": "odroczony",
                                                "telefon_retry_at": _retry,
                                                "telefon_proby": firestore.Increment(1),
                                                "telefon_last_op": op_name,
                                                "telefon_pz": parse_pz(ai_text) or _ccp.get("telefon_pz") or _ccp.get("pz") or "",
                                                "telefon_jezyk": _ccp.get("telefon_jezyk") or _ccp.get("jezyk") or _ccp.get("kraj") or "",
                                                "telefon_wsad": _ccp.get("pelna_linia_szturchacza", "") or _ccp.get("telefon_wsad", ""),
                                                "telefon_flagged_at": firestore.SERVER_TIMESTAMP,
                                            })
                            except Exception:
                                pass  # nie wywracaj sesji
    ...
    if st.session_state.get("chat_started") and _cc_phone.get("_phone_mode"):
        ...
        if _tel_dzwon_pz:
            ...
            st.markdown("**📞 Wynik telefonu:**")
            _pc1, _pc2, _pc3 = st.columns(3)
            with _pc1:
                if st.button("📵 Nie odebrał", key="_tel_nieodeb"):
                    st.session_state.messages.append({"role": "user", "content": f"SESJA WYNIK {_pnr} – TEL_nieodeb"})
                    st.rerun()
            with _pc2:
                if st.button("📨 Poczta", key="_tel_poczta"):
                    st.session_state.messages.append({"role": "user", "content": f"SESJA WYNIK {_pnr} – TEL_poczta"})
                    st.rerun()
            with _pc3:
            ...
            # ── KROK: telefon wykonany / niepotrzebny, AI wydał TAG → ZAKOŃCZ CASE ──
            st.markdown("---")
            _ph_odroczony = bool(_ph_wynik and _ph_wynik != "konkret")
            _doc = _cc_phone.get("_doc_id")
            if _ph_odroczony:
                st.info("📞 Telefon odroczony — sprawa zostaje w woreczku na ponowną próbę (+2h).")
                if st.button("▶️ Następny (zostaje w woreczku)", type="primary", key="_tel_finish_odr"):
                    for _k in ("messages", "ew_current_case", "ew_reverse_active", "ew_reverse_no_pool",
                               "ew_reverse_doc_id", "ew_reverse_grupa", "current_start_pz", "chat_nrzam"):
                        st.session_state.pop(_k, None)
                    st.session_state.chat_started = False
                    st.session_state._autopilot_loaded = False
                    st.session_state.ew_wsad_ready = ""
                    st.success("📞 Telefon odroczony — sprawa w woreczku.")
                    st.rerun()
            else:
                if st.button("✅ Zakończ case (telefon owocny) → następny", type="primary", key="_tel_finish_done"):
                    if _doc and not _cc_phone.get("_reverse_no_pool"):
                        ...
                        try:
                            ...
                            pass
                    # NIE wołamy log_stats tutaj — auto-log przy tagu (gdzie AI wydał c#/pz=) już policzył sesję;
                    # ponowne liczenie dublowałoby sessions_completed dla case'a telefonicznego.

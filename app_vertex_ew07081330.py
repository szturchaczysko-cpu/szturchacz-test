...
def ew_log_ponaglenia_bota(numer, grupa=None):
    ...
    for _p in _posty:
        ...
        try:
            _juz = db.collection(col("ew_phone_log")).document(_dzien).collection("delegacje") \
                     .where("numer_zamowienia", "==", _nr).limit(30).stream()
            ...
            db.collection(col("ew_phone_log")).document(_dzien).collection("delegacje").add({
                ...
                "typ": ("zlecenie" if _jest_del else "ponaglenie"),
...
def ew_log_deleg(op_name, numer, grupa, do_kogo, jezyk=None, pz=None,
                 id_postu=None, link=None, dzwoniacy=None, recznie=False):
    ...
    try:
        ...
        for _i_t in range(14):
            ...
            if any((d.to_dict() or {}).get("do_kogo") == (do_kogo or "") for d in _hist):
                _typ = "ponaglenie"
                break
    except Exception:
        pass
                                    ...
                                    for _idx_fw, fw in enumerate(forum_result["forum_writes"]):
                                        if fw.get("success"):
                                            ...
                                            if (_nrzam_e2 and _ud_fw.lower().startswith("telefoni")
                                                    and _to_prosba_o_tel
                                                    and not _to_raport and not _to_anul
                                                    and not _to_koniec_tel):

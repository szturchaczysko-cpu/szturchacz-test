...
def _wykryte_odp_telefonistow(forum_ctx):
    ...
    for _ln in str(forum_ctx).split("\n"):
        ...
        if _aut.lower().startswith("telefoni"):
            _out.append((_aut, _m.group(1) or "", _m.group(2) or ""))
    return _out


def ew_apply_tel_forum_wynik(ai_text, grupa_sprawy=None):
    """AI ocenia wpis telefonistki i wypisuje:
       TEL_FORUM_WYNIK=<brak_telefonu|brak_kontaktu|kontakt_bez_konkretu|przelozenie|konkret>[|KURIER]
    Telefon zapisujemy TYLKO gdy wpis RELACJONUJE rozmowę (wartość != brak_telefonu).
    'przyjmę / zadzwonię później / brak numeru' → brak_telefonu → żaden telefon nie powstaje."""
    if not ai_text:
        return None
    _m = re.search(r'TEL_FORUM_WYNIK\s*=\s*([a-zA-Z_]+)(\s*\|\s*KURIER)?', ai_text)
    if not _m:
        return None
    _w = _m.group(1).strip().lower()
    if _w not in ("brak_telefonu", "brak_kontaktu", "kontakt_bez_konkretu", "przelozenie", "konkret"):
        return None
    _kand = st.session_state.get("_tel_forum_kandydaci") or []
    if not _kand:
        return None
    _k = _kand.pop(0)                       # najstarszy nierozliczony wpis telefonistki
    st.session_state["_tel_forum_kandydaci"] = _kand
    # licznik dedupe na sprawie rośnie ZAWSZE — także przy brak_telefonu (wpis rozliczony).
    try:
        if _k.get("doc_id"):
            db.collection(col("ew_cases")).document(_k["doc_id"]).update(
                {"tel_forum_zalogowane": firestore.Increment(1)})
    except Exception:
        pass
    if _w == "brak_telefonu":
        return None                          # wpis nie relacjonuje rozmowy → brak telefonu
    _kur = bool(_m.group(2))
    try:
        db.collection(col("ew_phone_log")).document(_k["dzien"]).collection("calls").add({
            "numer_zamowienia": _k.get("numer", ""),
            "operator": _k.get("grupa_tel", "?"),     # nazwa GRUPY telefonistek (bez imion)
            "grupa": grupa_sprawy or _k.get("grupa_sprawy") or "?",
            "wynik": _w,
            "efektywny": _w in ("konkret", "przelozenie"),
            "kurier_ustalony": _kur,
            "zrodlo": "telefonista_zewn",
            "pz": "",
            "data_str": _k["dzien"],                  # REALNA data wpisu z forum
            "godzina": _k["godzina"],                 # REALNA godzina wpisu z forum
            "wykryte_dnia": datetime.now(pytz.timezone('Europe/Warsaw')).strftime("%Y-%m-%d"),
            "created_at": firestore.SERVER_TIMESTAMP,
        })
    except Exception:
        return None
...
def ew_log_telefonista_calls(numer, forum_ctx, doc_id=None, grupa=None):
    """Zbiera NOWE wpisy telefonistek jako KANDYDATÓW. Telefon powstanie dopiero po ocenie
    treści przez AI (TEL_FORUM_WYNIK) — patrz ew_apply_tel_forum_wynik."""
    _aut = _wykryte_odp_telefonistow(forum_ctx)
    if not _aut or not doc_id:
        return 0
    try:
        _juz = int((db.collection(col("ew_cases")).document(doc_id).get().to_dict() or {})
                   .get("tel_forum_zalogowane", 0))
    except Exception:
        return 0
    _nowe = _aut[_juz:]
    if not _nowe:
    ...
    _kand = []
    for _grupa_tel, _dfor, _gfor in _nowe:
        _kand.append({
            "grupa_tel": _grupa_tel,
            "numer": str(numer or "").strip(),
            "dzien": _dfor or _today,             # data wpisu z forum
            "godzina": _gfor or datetime.now(_tz).strftime("%H:%M"),
            "doc_id": doc_id,
            "grupa_sprawy": grupa or "?",
        })
    st.session_state["_tel_forum_kandydaci"] = _kand
    return len(_kand)
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            try:
                                ...
                                _w_forum = ew_apply_tel_forum_wynik(ai_text, operator_grupa)
                                if _w_forum:

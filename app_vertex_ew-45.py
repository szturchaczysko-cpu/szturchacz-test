...
with st.sidebar:
    ...
    # ── 📞 Woreczek telefoniczny (moduł Telefony) — pobierz → checklista PZ → zapisz wynik ──
    with st.expander("📞 Woreczek telefoniczny", expanded=False):
        st.caption("Sprawy z delegowanym telefonem (FORUM_TEL=czekam_wynik). Przy każdej: zadzwoń wg "
                   "checklisty dobranej do etapu, zaznacz wynik i kliknij Zapisz telefon — przy "
                   "konkrecie sprawa znika z woreczka. Telefon to OSOBNY ruch (nie wlicza się do przerobionych).")
        try:
        ...
        if not _woreczek:
            ...
            _wyn_opts = [
                ...
                "✅ Konkret — sprawa ruszyła (data / decyzja)",
            ]
            _wyn_map = {
                _wyn_opts[0]: "brak_kontaktu",
                _wyn_opts[1]: "kontakt_bez_konkretu",
                _wyn_opts[2]: "przelozenie",
                _wyn_opts[3]: "konkret",
            }
            for _wd in _woreczek:
                ...
                st.caption("Do ustalenia na telefonie (★ = popycha sprawę):")
                for _line in _phone_checklist_for(_wpz):
                    st.markdown(f"&nbsp;&nbsp;• {_line}")
                _wyn_label = st.radio("Wynik rozmowy:", _wyn_opts, key=f"_wor_wyn_{_wd.id}")
                _wkur = st.checkbox("💎📞 Doprowadził do ZLECENIA KURIERA (diamentofon)", key=f"_wor_kur_{_wd.id}")
                if st.button("💾 Zapisz telefon", type="primary", key=f"_wor_save_{_wd.id}"):
                    _wv = _wyn_map.get(_wyn_label, "kontakt_bez_konkretu")
                    try:
                        ew_log_phone(op_name, _wnum, _wv, grupa=_wgrp, kurier_ustalony=_wkur,
                                     zrodlo="operator_dzwoniacy", pz=_wpz)
                        # konkret → telefon zrobił swoje, sprawa znika z woreczka.
                        # reszta (nieodebrany / bez ustaleń / przełożenie) → zostaje (oddzwonić / 2h).
                        if _wv == "konkret":
                            db.collection(col("ew_cases")).document(_wd.id).update({"telefon_do_wykonania": False})
                            _msg = "✅ sprawa znika z woreczka"
                        else:
                            _msg = "↩️ zostaje w woreczku (oddzwonić)"
                        _dia = " + 💎📞" if (_wkur and _wv == "konkret") else ""
                        st.success(f"Zapisano telefon {_wnum} — {_wv}{_dia}. {_msg}.")
                        st.rerun()
                    except Exception as _e:
                        st.error(f"❌ Błąd zapisu: {_e}")
                if st.button("🗑️ Tylko usuń z woreczka (bez zapisu telefonu)", key=f"_wor_drop_{_wd.id}"):
                    try:
                        db.collection(col("ew_cases")).document(_wd.id).update({"telefon_do_wykonania": False})
                        st.success(f"Usunięto {_wnum} z woreczka (bez zapisu telefonu).")
                        st.rerun()
                    except Exception as _e:
                        st.error(f"Błąd: {_e}")
                st.markdown("---")

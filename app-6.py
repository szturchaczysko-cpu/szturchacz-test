...
def ew_log_phone(op_name, numer, wynik, grupa=None, kurier_ustalony=False,
                 ...
                 tresc=None, link=None, post_id=None):
...
def ew_log_deleg(op_name, numer, grupa, do_kogo, jezyk=None, pz=None,
    ...
    _typ = "zlecenie"
    try:
        _hist = db.collection(col("ew_phone_log")).document(today).collection("delegacje") \
                  .where("numer_zamowienia", "==", str(numer or "").strip()).limit(10).stream()
                ...
                for model_id in models_to_try:
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            if _tel_wynik:
                                _ust_m = re.search(r'USTALENIA\s*=\s*([^\n;]+)', ai_text)
                                _tresc_rozmowy = " ".join(filter(None, [
                                    (_ust_m.group(1) if _ust_m else ""),
                                    (re.search(r'TEL_odeb\s*:\s*([^\n]+)', ai_text).group(1)
                                     if re.search(r'TEL_odeb\s*:\s*([^\n]+)', ai_text) else ""),
                                ]))
                            ...
                            try:
                                ...
                                if (_tel_wynik and _nr_phone and _dzwonil_sam and not _wynik_z_forum
                                        and _ok_lang
                                    ...
                                    if True:
                                        ...
                                        _kur_ust = bool(_kp_m) or bool(locals().get("_kur_tresc"))

...
# Cache: który format zapytania GetPostTree akceptuje serwer (ustalany przy 1. udanym odczycie).
# Forum API potrafiło zmienić wymagania (406 Not Acceptable na „starym" POST). Zamiast zgadywać,
# forum_read PRÓBUJE kilku formatów i zapamiętuje działający — bez ręcznego testu i bez nowych plików.
_GETPOSTTREE_VARIANT = None


def _gpt_build_calls(root_id, branch_id, leaf_id, login_requested_by, login_acting_as, member_type_acting_as, page):
    """Warianty zapytania GetPostTree (różne formaty) dla danej strony → dict {nazwa: funkcja_request}."""
    url = f"{FORUM_API_BASE}/api/wpisy/GetPostTree"
    h = _headers()                                   # Content-Type + Authorization (bez Accept) = wariant „legacy"
    ha = dict(h); ha["Accept"] = "application/json"  # najczęstszy fix na 406 Not Acceptable

    lrb = login_requested_by or FORUM_USER
    body = {
        "root": root_id, "branch": branch_id, "leaf": leaf_id, "WholePage": None,
        "LoginRequestedBy": lrb, "PagingInfo": {"CurrentPage": page},
    }
    if login_acting_as:
        body["LoginActingAs"] = login_acting_as
        body["MemberTypeActingAs"] = member_type_acting_as if member_type_acting_as else (
            1 if _is_individual_user(login_acting_as) else 2)
    body_min = {k: v for k, v in body.items() if v is not None}     # bez null root/leaf/WholePage
    _cc = {"root": "Root", "branch": "Branch", "leaf": "Leaf"}
    body_camel = {_cc.get(k, k): v for k, v in body_min.items()}    # pola drzewa CamelCase
    params = {k: v for k, v in {
        "root": root_id, "branch": branch_id, "leaf": leaf_id,
        "LoginRequestedBy": lrb, "CurrentPage": page}.items() if v is not None}

    return {
        "post_accept": lambda: requests.post(url, headers=ha, json=body, timeout=30),
        "post_min":    lambda: requests.post(url, headers=ha, json=body_min, timeout=30),
        "post_camel":  lambda: requests.post(url, headers=ha, json=body_camel, timeout=30),
        "get":         lambda: requests.get(url, headers=ha, params=params, timeout=30),
        "post_legacy": lambda: requests.post(url, headers=h, json=body, timeout=30),
    }
...
def forum_read(branch_id=None, root_id=None, leaf_id=None, max_pages=5,
    ...
    """Czytanie drzewka (GetPostTree) — ODPORNE na zmianę formatu API.
    Próbuje kilku formatów zapytania, używa PIERWSZEGO z HTTP 200 i zapamiętuje go na kolejne odczyty.
    Jeśli ŻADEN format nie przejdzie → to NIE format, tylko uprawnienia/endpoint → temat dla IT.

    PRYWATNOŚĆ (brief §5.2): autoryzacja przez bearer; tożsamość w body (LoginRequestedBy / LoginActingAs).
    """
    global _GETPOSTTREE_VARIANT
    _ORDER = ["post_accept", "post_min", "post_camel", "get", "post_legacy"]
    all_posts = []
    thread_title = ""

    for page in range(1, max_pages + 1):
        calls = _gpt_build_calls(root_id, branch_id, leaf_id,
                                 login_requested_by, login_acting_as, member_type_acting_as, page)
        # Najpierw znany działający format, potem reszta (samo-naprawa, gdyby API znów zmieniło wymagania).
        order = ([_GETPOSTTREE_VARIANT] if _GETPOSTTREE_VARIANT in calls else []) + \
                [k for k in _ORDER if k != _GETPOSTTREE_VARIANT]
        ...
        for vname in order:
            try:
                ...
                last_err = f"{vname}: wyjątek {e}"
                continue
            if resp.status_code != 200:
                last_err = f"{vname}: HTTP {resp.status_code}"
                continue
            try:
                d = resp.json()
            except Exception:
                last_err = f"{vname}: HTTP 200 ale nie-JSON"
                continue
            # HTTP 200 = format ZAAKCEPTOWANY (status != SUCCESS to już kwestia danych, nie formatu).
            data = d
            if _GETPOSTTREE_VARIANT != vname:
                _GETPOSTTREE_VARIANT = vname
                _flog(f"GetPostTree: działający format = '{vname}' (zapamiętany na kolejne odczyty)")
            break

        if data is None:
            if page == 1:
                _flog(f"GetPostTree: WSZYSTKIE formaty nieudane ({last_err}) → to NIE format requestu, "
                      f"prawdopodobnie uprawnienia tokenu / endpoint wyłączony → temat dla IT.")
                return {"success": False, "error": f"GetPostTree all-variants-failed: {last_err}"}
            break

...
# Ostatnio odczytane wpisy wątku, per numer zamówienia — pozwalają powiązać ODPOWIEDŹ
# z konkretnym NASZYM wpisem (Do_Odpid / Hierarchy), zamiast zgadywać po godzinach.
_OSTATNIE_POSTY = {}
_KLUCZ_SESJI = "_forum_ostatnie_posty"
...
def _zapamietaj_posty(nrzam, posty):
    ...
    try:
        ...
        _mapa[_nr] = posty
        # trzymamy ostatnie 20 spraw, żeby sesja nie puchła
        if len(_mapa) > 20:
            for _k in list(_mapa.keys())[:-20]:
                _mapa.pop(_k, None)
        st.session_state[_KLUCZ_SESJI] = _mapa
    except Exception:
        pass


def ostatnie_posty(nrzam):
    """Struktura wpisów z ostatniego odczytu wątku — z pamięci procesu albo z sesji."""
    _nr = str(nrzam or "").strip()
    _p = _OSTATNIE_POSTY.get(_nr)
    if _p:
        return _p
    try:
...
def _rozbij_autora(nazwa, osoba_pole=""):
    """Autor bywa podawany jako 'TELEFONIŚCI_DE(anna)' w JEDNYM ciągu, a osobne pole
    z osobą bywa puste. Rozbijamy nawias, żeby imię nie przepadało."""
    _n = str(nazwa or "").strip()
    _os = str(osoba_pole or "").strip()
    _m = re.match(r'^([^(]+)\(([^)]+)\)\s*$', _n)
    if _m:
        _n = _m.group(1).strip()
        if not _os:
            _os = _m.group(2).strip()
    return {"Autor": _n, "Osoba": _os}


def _czas_lokalny(v):
    """🟥 Forum zwraca DateAdd w czasie UTC — interfejs pokazuje czas lokalny.
    Bez przeliczenia wpisy z forum były o 1-2h wcześniejsze niż nasze zapisy, przez co
    rozmowa wyglądała na wykonaną PRZED zleceniem. Przeliczamy przez strefę (nie na sztywno),
    żeby zmiana czasu w październiku niczego nie rozjechała."""
    if not v:
        return "?"
    _s = str(v).replace("T", " ").strip()
    for _fmt, _n in (("%Y-%m-%d %H:%M:%S", 19), ("%Y-%m-%d %H:%M", 16)):
        try:
            _d = datetime.strptime(_s[:_n], _fmt)
            break
        except Exception:
            _d = None
    if _d is None:
        return _s[:16]
    try:
        _utc = _d.replace(tzinfo=timezone.utc)
        if _TZ_PL is not None:
            return _utc.astimezone(_TZ_PL).strftime("%Y-%m-%d %H:%M")
        # awaryjnie: przesunięcie letnie/zimowe wyliczone z reguły UE
        _off = 2 if 3 <= _d.month <= 10 else 1
        return (_utc + timedelta(hours=_off)).strftime("%Y-%m-%d %H:%M")
    except Exception:
        return _s[:16]
...
def forum_write(post_id, do_odp_id, user_do, tresc, user_do_type=1, user_od=None, ai_user=None, tytul=None):
    ...
    try:
        ...
        # DIAGNOSTYKA: pokaz TRESC odpowiedzi forum (nie tylko kod HTTP). Przy 401 mowi, czy to
        # nieważny/wygasly TOKEN (auth), czy nieautoryzowany ODBIORCA/grupa. Bez tego widac samo "401".
        _body = ""
        try:
            if e.response is not None:
                _body = e.response.text[:800]
        except Exception:
            _body = ""
        _flog(f"WRITE HTTP ERROR: {e} | odpowiedz forum: {_body}")
        return {"success": False, "error": f"{e} | odpowiedz forum: {_body}"}
    except Exception as e:
        _flog(f"WRITE ERROR: {e}")
...
def parse_forum_markers(ai_response):
    ...
    for m in FORUM_MARKER_PATTERN.finditer(ai_response):
        ...
        if action == "write":
            ...
            # 🟥 Model bywa niedokładny — wpisuje tam tekst, pusty ciąg albo numer z opisem.
            #    Bez zabezpieczenia zamiana na liczbę wywalała CAŁĄ aplikację w trakcie
            #    przerabiania sprawy. Niepoprawna wartość = brak podpięcia, nie awaria.
            marker["do_odp_id"] = None
            if "do_odp_id" in params:
                _cyfry = re.sub(r"\D", "", str(params["do_odp_id"] or ""))
                if _cyfry:
                    try:
                        marker["do_odp_id"] = int(_cyfry)
                    except Exception:
                        marker["do_odp_id"] = None
            marker["user_do"] = params.get("user_do", None)
            marker["tytul"] = params.get("tytul", None)
        elif action == "read":
            marker["forum_id"] = None
            _c_forum_id = re.sub(r"\D", "", str(params.get("forum_id") or ""))
            if _c_forum_id:
                try:
                    marker["forum_id"] = int(_c_forum_id)
                except Exception:
                    marker["forum_id"] = None if "forum_id" in params else None
            marker["cel"] = params.get("cel", "")
...
def execute_forum_actions(ai_response, forum_memory=None, user_od=None, ai_user=None,
    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            # 🟥 MARKER NIEKOMPLETNY — SPRAWDZANE OD RAZU, przed jakąkolwiek obróbką.
            #    Model bywa niedokładny: wypisuje marker bez celu albo bez treści (czasem
            #    stwierdzając w SELF-CHECK, że wpisu nie ma). Wysyłka nie ma wtedy sensu,
            #    a operator dostawał prośbę o wklejenie PUSTEJ treści.
            if not str(cel or "").strip() or not str(tresc or "").strip():
                _braki = []
                if not str(cel or "").strip():
                    _braki.append("cel (wątek)")
                if not str(tresc or "").strip():
                    _braki.append("treść wpisu")
                _flog(f"MARKER NIEKOMPLETNY: brakuje {', '.join(_braki)} — pomijam")
                forum_writes.append({
                    "success": False,
                    "error": f"Model nie podał: {', '.join(_braki)}. Wpis NIE powstał "
                             f"i nie ma czego wklejać — poproś Szturchacza o ponowne "
                             f"wygenerowanie odpowiedzi dla tej sprawy.",
                    "cel": cel or "", "tresc": "", "tresc_skrot": "",
                    "user_do": user_do or "", "marker_niekompletny": True,
                })
                continue
            ...
            if _watek_w and _watek_w in _watki_kotwicy():
                ...
                try:
                    ...
                    if _kand_w:
                        ...
                        _flog(f"PODPIECIE: nrzam={_nrzam_w} cel={cel} -> do_odp_id={do_odp_id}")
                except Exception as _e_w:
                    _flog(f"PODPIECIE BLAD: {_e_w}")
            ...
            # 🟥 PODPIĘCIE MOŻE SIĘ NIE UDAĆ Z POWODU UPRAWNIEŃ. Wątki półprywatne bywają
            #    niewidoczne dla konta, z którego piszemy — forum odmawia odpowiedzi na wpis,
            #    którego nie wolno zobaczyć. Wtedy zakładamy NOWY podwątek, zamiast nie wysłać nic.
            if (not result.get("success")) and do_odp_id:
                _err_txt = str(result.get("error") or result.get("message") or "")
                if ("uprawnie" in _err_txt.lower() or "406" in _err_txt
                    ...
                    _is_new_subthread = True
            result["cel"] = cel
            result["tresc_skrot"] = tresc[:100] if tresc else ""
            # 🟥 ODBIORCA i PEŁNA TREŚĆ w wyniku — bez nich wywołujący nie wie, czy wpis
            #    szedł do Telefonistów, i nie zapisze go do rejestru telefonów.
            result["user_do"] = user_do or ""
            result["tresc"] = tresc or ""
            ...
            if result.get("success"):
                if result["count"] == 0:
                    ...
                    for p in result["posts"]:
                        date_str = _czas_lokalny(p.get('DateAdd'))   # UTC z forum -> czas lokalny
                        context_parts.append(
                            f"[{date_str}] {p['UserAddName']}{('(' + str(p.get('UserOdInGroup')) + ')') if p.get('UserOdInGroup') else ''} → {p['UserToName']}: "
...
_FORUM_THREADS_PROD = {
    ...
    # ALIASY BEZPIECZEŃSTWA: UK i PL mają WSPÓLNY wątek (5691). Gdyby AI skleiło nazwę
    # "CZATOSZTUR_" + kod kraju, wpis i tak trafi tutaj zamiast wywalić się na "Nieznany cel".
    "CZATOSZTUR_UK": {
        "post_id": 5691, "korzen_id": None,
        "grupa": "OPERATORZY_UK", "grupa_type": 2,
        "opis": "Alias → wspólny wątek UK/PL (5691)",
    },
    "CZATOSZTUR_PL": {
        "post_id": 5691, "korzen_id": None,
        "grupa": "OPERATORZY_PL", "grupa_type": 2,
        "opis": "Alias → wspólny wątek UK/PL (5691)",
    },
    "szturchacz_błędy": {
    ...
    try:
        ...
        for cel, info in memory.items():
            ...
            if result.get("success") and result.get("posts"):
                ...
                try:
                    _zapamietaj_posty(numer_zamowienia, [{
                        "Id": p.get("Id"),
                        "Do_Odpid": p.get("Do_Odpid"),
                        "Hierarchy": p.get("Hierarchy", ""),

...
def log_diamond(db, diamond_prefix, numer_zamowienia, operator, source_type,
    ...
    Dedup: pierwszy zapis dla danej pary (data, numer) wygrywa — kolejne ignorowane.
    ...
    """
    if db is None or not numer_zamowienia:
        return
    try:
        ...
        _flog(f"DIAMOND LOGGED: {date_str}/{numer_zamowienia} | op={operator} | src={source_type} | cel={cel} | pz={pz} | bump={bump}")
    except Exception as e:
    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            if result.get("success"):
                ...
                # === DIAMENT: zlecenie kuriera (cel=AUTOS_KURIERZY) i apka zdecydowała że to diament ===
                # Filtr: cel == AUTOS_KURIERZY (lub testowy KURIER_test) AND is_diamond_candidate=True
                # is_diamond_candidate = apka parsuje ai_text i sprawdza PZ=PZ6 + bump=0
                # Dlaczego apka, nie moduł: bump i PZ żyją w tagu C# w ai_text, nie w markerze FORUM_WRITE.
                # Apka ma ai_text w scope, moduł go nie dostaje — przekazujemy gotową decyzję.
                if (cel in ("AUTOS_KURIERZY", "KURIER_test")
                        and is_diamond_candidate
                        and db is not None):

...
def forum_write(post_id, do_odp_id, user_do, tresc, user_do_type=1, user_od=None, ai_user=None, tytul=None):
    ...
    ai_user: nick operatora który stoi za botem (np. "klaudia", "kasia_k", "oliwia_m").
             Trafia do pola UserRzeczywisty w payloadzie — to ono jest źródłem widoku
             "GRUPA (nick)" na forum. Pole AiUser jest ZAWSZE = FORUM_USER
             ("chatoszturek") — bot AI, zgodnie z kontraktem API (AiUser nie może
             być człowiekiem; wpychanie nicku operatora do AiUser powodowało
             odrzucanie requestów, case oliwia_m).
    """
    if user_od is None:
        user_od = FORUM_USER
    ...
    payload = {
        ...
        "subThread": {
            ...
            "private": False,
            "AiGenerated": 1,
            # AiUser = ZAWSZE bot AI (kontrakt API: AiUser nie może być człowiekiem).
            # Werdykt Sylwii 12.06: literalnie "chatoszturek" (= FORUM_USER).
            "AiUser": FORUM_USER,
            # UserRzeczywisty = człowiek/operator stojący za wpisem — źródło widoku
            # "GRUPA (nick)" na forum. Dla autopilota apki przekazują "chatoszturek".
            "UserRzeczywisty": ai_user if ai_user else FORUM_USER,
        }
    }

...
def _klasyfikuj_wpis(tekst):
    ...
    Zwraca (wynik, kurier, pewne) — 'pewne' mówi, czy da się rozstrzygnąć bez modelu.

    Kolejność ma znaczenie: najpierw brak rozmowy, potem brak kontaktu, potem martwy numer,
    potem konkret, na końcu umówiony kolejny kontakt."""
    _t = " " + re.sub(r"\s+", " ", str(tekst or "").lower()) + " "
    if not _t.strip():
        return None, False, False

    def _ma(*frazy):
        return any(f in _t for f in frazy)

    # 1) wpis NIE relacjonuje rozmowy
    if _ma("przyjmuj", "przyjmę", "przyjmuje", "zadzwonię", "zadzwonie później", "zadzwonimy",
           "biorę", "biore na siebie", "w trakcie", "będę dzwonić", "bede dzwonic"):
        # uwaga: „zadzwonię" to ZAPOWIEDŹ, nie relacja — szukamy form przeszłych
        if not _ma("dzwonilam", "dzwoniłam", "dzwonili", "dzwonilismy", "rozmaw",
                   "odebra", "nie odbiera", "polaczenie", "połączenie"):
            return "brak_telefonu", False, True

    # 2) nie odebrał / poczta
    if _ma("nie odbiera", "nie odebra", "nie odbiral", "nie odbierał", "brak kontaktu",
           "poczta glosowa", "poczta głosowa", "sekretarka", "no answer", "not answering",
           "keine antwort", "nikt nie odbiera", "nie ma kontaktu", "nieodebrany"):
        return "nieodebral", False, True

    # 3) martwy numer / rozmowa bez wartości
    if _ma("zly numer", "zły numer", "bledny numer", "błędny numer", "incorrect number",
           "wrong number", "nie ten numer", "numer nalezy", "numer należy", "inna osoba",
           "dogadac", "dogadać", "nikt nic nie wie", "nie na temat",
           "nie zna zamowienia", "nie zna zamówienia", "falsche nummer"):
        return "nieproduktywne", False, True

    # 4) konkret — data odbioru, gotowość, kurier
    _kur = _ma("kurier", "kuriera", "kurierem", "dpd", "dhl", "gls", "fedex", "ups")
    if _ma("zamowion", "zamówion", "odbior", "odbiór", "gotow", "spakowan", "wysyla", "wysyła",
           "wyśle", "wysle", "bedzie gotowa", "będzie gotowa", "umowion", "umówion") \
       or (_kur and _ma("ustal", "zamow", "zamów", "na dzien", "na dzień")):
        # „będzie gotowa za tydzień, kontakt 07.08" to termin NA termin, nie konkret
        if _ma("kontakt", "oddzwon", "ponownie", "za tydzien", "za tydzień", "przypomn"):
            return "ustalono_termin_na_termin", _kur, True
        return "ustalono_termin", _kur, True

    # 5) umówiony kolejny kontakt
    if _ma("kontakt", "oddzwon", "prosi o telefon", "prosi o kontakt", "za tydzien", "za tydzień",
           "w przyszlym tygodniu", "w przyszłym tygodniu", "przypomn", "pozniej", "później"):
        return "ustalono_termin_na_termin", False, True

    return None, False, False        # niejednoznaczne → do oceny
...
def ew_log_telefonista_calls(numer, forum_ctx, doc_id=None, grupa=None):
    ...
    for _p in _posty:
        ...
        _wyn_auto, _kur_auto, _pewne = _klasyfikuj_wpis(_p.get("Tekst"))
        if _wyn_auto == "brak_telefonu":
            continue                       # wpis nie relacjonuje rozmowy — nie ma telefonu
        if not _pewne:
            _wyn_auto = None
        try:
            # 🟥 Rozmowa NALEŻY DO DNIA, W KTÓRYM OPERATOR JĄ PRZEROBIŁ — wtedy weszła do systemu.
            #    Godzina i data z forum zostają osobno, do liczenia czasu reakcji telefonistki.
            _res = db.collection(col("ew_phone_log")).document(_today).collection("calls").add({
                "numer_zamowienia": str(numer or "").strip(),
                "operator": _aut,
                "osoba": _osoba,
                "grupa": grupa or "?",
                ...
                "pz": "",
                "data_str": _today,            # dzień PRZEROBIENIA — po nim filtrują tabele
                "godzina": _godz,              # godzina wpisu z forum
                "forum_data": _dzien,          # realna data wpisu z forum
                "forum_godzina": _godz,
            ...
            _pending.append({"dzien": _today, "id": _res[1].id, "numer": str(numer or "").strip()})

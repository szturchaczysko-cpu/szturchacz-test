...
if TEST_MODE:
    seeded_cfg = {
        ...
        "assigned_key_index": cfg.get("assigned_key_index", 1),
        "tel": cfg.get("tel", False),
    }
    if not cfg.get("role"):
        seeded_cfg["role"] = "Operatorzy_DE"
    cfg_ref.set(seeded_cfg, merge=True)
    cfg = {**cfg, **seeded_cfg}
...
if "_last_forum_hash" not in st.session_state:

...
if "chat_nrzam" not in st.session_state:
    ...
    st.session_state._last_forum_hash = None
                ...
                if is_reverse:
                    ...
                    if wsad:
                        ...
                        if FORUM_ENABLED and nrzam:
                            forum_ctx = auto_load_forum_context(db, col, nrzam, login=cfg.get('role', 'Operatorzy_DE'))
                            if forum_ctx:
                                wsad = wsad + "\n\n" + forum_ctx
...
if st.session_state.get("czyste_okno", False) and not st.session_state.get("chat_started", False):
    ...
    if st.button("🚀 Analizuj", type="primary"):
        if czyste_wsad.strip():
            ...
            if FORUM_ENABLED and nrzam:
                forum_ctx = auto_load_forum_context(db, col, str(nrzam), login=cfg.get('role', 'Operatorzy_DE'))
                if forum_ctx:
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            if FORUM_ENABLED and ("[FORUM_WRITE|" in ai_text or "[FORUM_READ|" in ai_text):
                                # Guard: nie wykonuj markerów dwa razy dla tej samej odpowiedzi
                                _response_hash = hashlib.md5(ai_text[:200].encode()).hexdigest()[:12]
                                if st.session_state.get("_last_forum_hash") == _response_hash:
                                    pass  # już wykonane — skip
                                else:
                                    st.session_state._last_forum_hash = _response_hash
                                    ...
                                    forum_result = execute_forum_actions(ai_text, forum_memory=_fm_e2, user_od=cfg.get('role', 'Operatorzy_DE'))
        ...
        if st.button("🚀 Rozpocznij analizę", type="primary"):
            if wsad_input:
                ...
                if FORUM_ENABLED and _nrzam_clean:
                    forum_ctx = auto_load_forum_context(db, col, _nrzam_clean, login=cfg.get('role', 'Operatorzy_DE'))
                    if forum_ctx:
                        wsad_input = wsad_input + "\n\n" + forum_ctx

...
with st.sidebar:
    ...
    if not czyste_okno:
        ...
        if show_current_case:
            ...
            st.markdown("---")
            # === ZAKOŃCZENIE — rozróżnienie standard vs wsad odwrotny ===
            _is_rev = case.get("_reverse_mode", False)
            _rev_type = case.get("_reverse_type", "")  # "WA" | "MAIL" | "FORUM"
            _btn_label = f"✅ Zakończ wsad odwrotny ({_rev_type}) → następny" if _is_rev else "✅ Zakończ → Następny"
            ...
            if st.button(_btn_label):
                ...
                if tag:
                    # Kanał ruchu: standard albo odwrotny (wa/mail/forum) — do trwałej kroniki
                    _channel = {"WA": "wa", "MAIL": "mail", "FORUM": "forum"}.get(_rev_type, "standard") if _is_rev else "standard"
                    # _reverse_no_pool = case zrobiony/spoza świeżej puli: NIE mutuj dokumentu i NIE licz
                    # ponownie do grupy (gz_), żeby nie zawyżać "przerobionych" grupy. Operator i tak
                    # dostaje kredyt za ruch odwrotny (kanał) — bo wykonał pracę.
                    _no_pool = case.get("_reverse_no_pool", False)
                    _grp_for_log = None if _no_pool else case.get("grupa")
                    # Case z puli (pierwsze zakończenie) → oznacz zakończony (NIE wróci do standardu).
                    if case.get("_doc_id") and not _no_pool:
                    ...
                    ew_log_completion(op_name, channel=_channel, grupa=_grp_for_log)
                    # Reset sesji (wspólny dla obu trybów)
                    st.session_state.messages = []
                    st.session_state.chat_started = False
                    st.session_state.current_start_pz = None
                    st.session_state._autopilot_loaded = False
                    ...
                    if _is_rev:
                        # Czysta karta — operator sam wkleja nowy wsad albo szuka przez "Szukaj case".
                        # ŻADNEGO auto-pobrania ze standardowej puli (tryb odwrotny ≠ standard).
                        st.session_state.ew_current_case = None
                        st.success(f"✅ Wsad odwrotny ({_rev_type}) zakończony. Czysta karta — rozpocznij kolejny.")
                        st.rerun()
                    else:

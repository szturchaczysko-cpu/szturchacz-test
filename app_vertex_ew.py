...
# ── REJESTR ZLECONYCH TELEFONÓW (zakładka w Wieżowcu) ──────────────────────────
# Append-only log: 1 dokument = 1 zlecony telefon (wpis do Telefoniści_*).
# Wypełniany automatycznie: przy delegacji (id postu, kiedy, do kogo, grupa, kto zlecił),
# potem przy wyniku telefonu (podjęte/przez kogo/status). Reszta uzupełniana ręcznie w Wieżowcu.
TEL_REG_COL = "ew_telefony_log"
TEL_STATUSY = [
    "ZLECONY", "NIE ODBIERA", "POCZTA GŁOSOWA", "ODEBRAŁ — USTALONO",
    "ODEBRAŁ — BEZ KONKRETU", "TERMIN NA TERMIN", "ZAMÓWIONY",
    "NIEAKTUALNY — ZMIANA KANAŁU", "ANULOWANY",
]
...
def tel_reg_add(nrzam, id_postu, link, do_kogo, grupa_op, zlecil, jezyk, pz, case_doc_id=None):
    """Zapis nowego zleconego telefonu (przy udanej delegacji na forum)."""
    try:
        db.collection(col(TEL_REG_COL)).add({
            "numer_zamowienia": str(nrzam or ""),
            "id_postu": str(id_postu or ""),
            "link": link or "",
            "do_kogo": do_kogo or "",
            "grupa_operatorska": grupa_op or "",
            "zlecil": zlecil or "",
            ...
            "podjete": False,
            "podjete_przez": "",
            "case_doc_id": case_doc_id or "",
            "data_zlecenia": firestore.SERVER_TIMESTAMP,
        })
    except Exception:
        pass  # rejestr nigdy nie wywraca sesji

def tel_reg_update_latest(nrzam, fields):
    """Aktualizuje NAJNOWSZY wpis rejestru dla danego zamówienia (wynik telefonu)."""
    try:
        _docs = list(db.collection(col(TEL_REG_COL))
                     .where("numer_zamowienia", "==", str(nrzam)).limit(25).stream())
        if not _docs:
            return
        def _kt(_d):
            _v = (_d.to_dict() or {}).get("data_zlecenia")
            try:
                return _v.timestamp()
            except Exception:
                return 0
        _docs.sort(key=_kt, reverse=True)
        db.collection(col(TEL_REG_COL)).document(_docs[0].id).update(fields)
    except Exception:
        pass
                                    ...
                                    for fw in forum_result["forum_writes"]:
                                        if fw.get("success"):
                                            ...
                                            if _nrzam_e2 and _ud_fw.lower().startswith("telefoni"):
                                                ...
                                                tel_reg_add(
                                                    nrzam=_nrzam_e2, id_postu=fw.get("FORUM_ID"), link=fw.get("link"),
                                                    do_kogo=_ud_fw, grupa_op=operator_grupa, zlecil=op_name,
                                                    jezyk=_tel_deleg_lang, pz=parse_pz(ai_text) or _ccx_reg.get("pz") or "",
                                                    case_doc_id=_ccx_reg.get("_doc_id"),
                                                )
                                        else:
    ...
    if st.session_state.get("chat_started") and _cc_phone.get("_phone_mode"):
        ...
        if _tel_dzwon_pz:
            ...
            def _wor_odroczony(_cmd):
                ...
                tel_reg_update_latest(_pnr, {
                    "podjete": True, "podjete_przez": op_name,
                    "podjete_at": firestore.SERVER_TIMESTAMP,
                    "status": "POCZTA GŁOSOWA" if "poczta" in (_cmd or "").lower() else "NIE ODBIERA",
                    "wykonane_at": firestore.SERVER_TIMESTAMP,
                })
            ...
            with _pc3:
                if st.button("✅ Odebrał — zapisz", key="_tel_odeb_send", type="primary"):
                    ...
                    tel_reg_update_latest(_pnr, {
                        "podjete": True, "podjete_przez": op_name,
                        "podjete_at": firestore.SERVER_TIMESTAMP,
                        "status": "ODEBRAŁ — USTALONO" if _ust_txt else "ODEBRAŁ — BEZ KONKRETU",
                        "ustalenia": _ust_txt or "",
                        "wykonane_at": firestore.SERVER_TIMESTAMP,
                    })
        ...
        try:
            ...
            if "SESJA WYNIK" in _pu and _docx and not _ccx.get("_reverse_no_pool"):
                if "TEL_NIEODEB" in _pu or "TEL_POCZTA" in _pu:
                    ...
                    tel_reg_update_latest(_ccx.get("numer_zamowienia") or st.session_state.get("chat_nrzam") or "", {
                        "podjete": True, "podjete_przez": op_name,
                        "podjete_at": firestore.SERVER_TIMESTAMP,
                        "status": "POCZTA GŁOSOWA" if "TEL_POCZTA" in _pu else "NIE ODBIERA",
                        "wykonane_at": firestore.SERVER_TIMESTAMP,
                    })

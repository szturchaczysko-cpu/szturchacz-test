                                ...
                                if forum_result["forum_writes"]:
                                    ...
                                    for fw in forum_result["forum_writes"]:
                                        if fw.get("success"):
                                            ...
                                            _ud_map = {}
                                            for _mm in re.finditer(r'\[FORUM_WRITE\|cel=([^|\]]+)\|[^\]]*?user_do=([^|\]]+)', ai_text):
                                                _ud_map[_mm.group(1).strip()] = _mm.group(2).strip()
                                            _ud_fw = _ud_map.get(str(fw.get("cel") or "").strip(), "")
                                            ...
                                            _ud_map_f = {}
                                            for _mf in re.finditer(r'\[FORUM_WRITE\|cel=([^|\]]+)\|[^\]]*?user_do=([^|\]]+)', ai_text):
                                                _ud_map_f[_mf.group(1).strip()] = _mf.group(2).strip()
                                            _ud_f = _ud_map_f.get(str(fw.get("fallback_cel") or fw.get("cel") or "").strip(), "") \

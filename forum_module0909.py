...
def execute_forum_actions(ai_response, forum_memory=None, user_od=None, ai_user=None,
    ...
    for marker in markers:
        if marker["type"] == "write":
            ...
            # 🟥 MARKER NIEKOMPLETNY — model bywa niedokładny i wypisuje marker bez celu
            #    albo bez treści. Wysyłka wtedy nie ma sensu, a operator dostawał prośbę
            #    o wklejenie PUSTEJ treści. Mówimy wprost, czego zabrakło.
            if not str(cel or "").strip() or not str(tresc or "").strip():
                ...
                _flog(f"MARKER NIEKOMPLETNY: brakuje {', '.join(_braki)}")
                forum_writes.append({
                    "success": False,
                    "error": f"Model nie podał: {', '.join(_braki)}. "
                             f"Wpis nie powstał — poproś Szturchacza o ponowne wygenerowanie.",
                    "cel": cel or "", "tresc": tresc or "", "tresc_skrot": "",

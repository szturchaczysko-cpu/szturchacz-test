...
def col(name):
    ...
    WYJĄTEK: ew_cases czyta z produkcji (bez prefixu)."""
    if name == "ew_cases":
        return name  # produkcyjne casy
...
if TEST_MODE:
    if not cfg or not cfg.get("role"):
        # Kopiuj config z produkcji
        _prod_cfg = db.collection("operator_configs").document(op_name).get().to_dict() or {}
        if _prod_cfg:
            cfg = _prod_cfg
        else:
            cfg = {
    ...
    cfg["prompt_url"] = "https://raw.githubusercontent.com/szturchaczysko-cpu/szturchacz-test/refs/heads/main/v4_forum.txt"

...
_RE_DIAMOND_KURIER = re.compile(r'Kurier\s*[:=]\s*(UPS|FEDEX|DB[_\s]?SCHENKER|DBSCHENKER)', re.IGNORECASE)
...
def _classify_typ_zlecenia(tresc):
    """Klasyfikacja typu zlecenia z treści posta: kurier / etykieta_ups_punkt / inne.

    Etykieta sprawdzana PRZED kurierem — treść etykiety zawiera też słowo KURIER.
    """
    if not tresc:
        return "inne"
...
def log_diamond(db, diamond_prefix, numer_zamowienia, operator, source_type,
    ...
    try:
        ...
        _flog(f"DIAMOND LOGGED: {date_str}/{numer_zamowienia} | op={operator} | src={source_type} | cel={cel} | typ={typ_zlecenia} | kat={kategoria_towaru} | kurier={kurier}")
    except Exception as e:

...
TEL_WYNIKI = ("ustalono_termin", "ustalono_termin_na_termin", "nieodebral", "nieproduktywne")
...
def _klasyfikuj_wpis(tekst):
    ...
    # 3) NIEPRODUKTYWNE — rozmowa/próba bez wartości, wymaga decyzji człowieka
    # 🟥 Martwy numer = BRAK KONTAKTU. Do rozmowy nie doszło, więc to nie jest
    #    nieudana rozmowa — telefonistka nie miała żadnej szansy.
    if _ma("nummer nicht vergeben", "kein anschluss", "nicht vergeben", "falsche nummer",
           ...
           "oba telefony nieprawidlowe", "numer nalezy", "numer należy"):
        return "nieodebral", False, True, _ozn
    ...
    if _ma("nieprawidlow",
           ...
           "na urlopie", "nie ma osoby", "nie ma nikogo", "nie ma dzis osoby",
    ...
    # 4) UMÓWIONY KOLEJNY KONTAKT — kotwica, prośba o telefon w terminie, brak gotowości
    # „nie ma osoby / trzeba jutro próbować" — rozmowa bez wartości, mimo słowa „jutro"
    if _ma("trzeba jutro", "nie ma dzis osoby", "nie ma dziś osoby"):
        return "nieproduktywne", False, True, _ozn
...
def ew_log_telefonista_calls(numer, forum_ctx, doc_id=None, grupa=None):
    ...
    for _p in _posty:
        ...
        _sygn_tresc = f"D|{_aut}|{_osoba}|{_tresc_norm}"
        if _tresc_norm and _sygn_tresc in _stare:
            continue
        _sygn = f"{_p.get('Id')}"
        if _sygn in _stare:
            continue
        ...
        if _wyn_auto == "brak_telefonu":
            continue                       # wpis nie relacjonuje rozmowy — nie ma telefonu
        if not _pewne:
            _wyn_auto = None
        try:
            ...
            _res = db.collection(col("ew_phone_log")).document(_dzien).collection("calls").add({
                ...
                "forum_data": _dzien,
                "forum_godzina": _godz,
                "przerobione_dnia": _today,    # kiedy MY się o niej dowiedzieliśmy
                ...
                for model_id in models_to_try:
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            if re.search(r'TEL_DZWON\s*=', ai_text):
                                    ...
                                    for _idx_fw, fw in enumerate(forum_result["forum_writes"]):
                                        if fw.get("success"):
                                            ...
                                            _to_raport = (bool(_tel_wynik) and _tel_wynik != "brak_kontaktu"
                                                          and _dzwonil_teraz)
                                            if _nrzam_e2 and _ud_fw.lower().startswith("telefoni") and not _to_raport:
                            ...
                            try:
                                ...
                                _klucz_log = f"{_nr_phone}|{_tel_wynik}|{len(_juz_zalog)}"

...
if "operator" not in st.session_state:
    ...
    op = st.selectbox("Operator:", ["Sylwia", "Emilia", "Oliwia", "Magda", "Ewelina", "Iwona", "Marlena", "EwelinaG", "Andrzej", "Marta", "Klaudia", "Kasia", "Romana"])
...
# Zawsze użyj testowego promptu forum — domyślny z test_admin_config + ewentualny override per operator
if TEST_MODE:
    # Default dla warstwy B (operatorzy testowi)
    _default_doc = db.collection("test_admin_config").document("default_prompt").get()
    _default_data = _default_doc.to_dict() or {}
    _default_url = _default_data.get("prompt_url", "https://raw.githubusercontent.com/szturchaczysko-cpu/szturchacz-test/refs/heads/main/v4_forum_weryfik.txt")
    _default_name = _default_data.get("prompt_name", "v4 forum weryfik")
    _default_filename = _default_data.get("prompt_filename", "v4_forum_weryfik.txt")
    
    # Override per operator (warstwa C — eksperymenty)
    _override_doc = db.collection("test_operator_overrides").document(op_name).get()
    _override_data = _override_doc.to_dict() or {}
    
    if _override_data.get("prompt_url"):
        cfg["prompt_url"] = _override_data["prompt_url"]
        cfg["prompt_name"] = _override_data.get("prompt_name", "override")
        cfg["prompt_filename"] = _override_data.get("prompt_filename", "override.txt")
        cfg["prompt_layer"] = "C"
        cfg["prompt_github_link"] = _override_data.get("prompt_github_link", "")
        # Toast o zmianie prompta (jeśli operator nie widział tej wersji)
        _last_seen_key = f"_last_seen_prompt_{op_name}"
        if st.session_state.get(_last_seen_key) != _override_data["prompt_url"]:
            st.toast(f"📄 Zmieniono prompt na: {cfg['prompt_name']}", icon="🧪")
            st.session_state[_last_seen_key] = _override_data["prompt_url"]
    else:
        cfg["prompt_url"] = _default_url
        cfg["prompt_name"] = _default_name
        cfg["prompt_filename"] = _default_filename
        cfg["prompt_layer"] = "B"
        cfg["prompt_github_link"] = _default_data.get("prompt_github_link", "")
    ...
    for d in all_free:
        ...
        if is_calculated and assigned_op == op_name:
            ...
            if other_tel == my_tel:
                ...
                # (operator może go pobrać gdy nie ma własnych przeliczonych)
                prio4.append(d)
        else:
            prio3.append(d)
...
def ew_get_fresh_case(grupa, op_name):
    ...
    2. Jeśli brak — fallback do CALCULATED (ale NIE forum_action)
    3. Pomija forum_action (czeka na uwolnienie)
    
    Cel: faza testów po zmianie prompta. Operator może sprawdzić jak nowy prompt 
    obsługuje case'y nieprzeliczone albo te które stary prompt już raz przeliczał.
    """
    skipped_ids = st.session_state.get("ew_skipped_ids", set())
    ...
    doc = None
    fresh_source = None  # "nieprzeliczony" / "calculated_recalc"
    for d in all_free:
        data = d.to_dict()
    ...
    # Update bazy: oznacz jako świeżak, wyczyść stare autopilot_messages żeby nie ładowało starej analizy
    update_data = {
        "status": "przydzielony",
        "assigned_to": op_name,
        "assigned_at": firestore.SERVER_TIMESTAMP,
    ...
    if fresh_source == "calculated_recalc":
        # Wyczyść stare autopilot_messages żeby świeżak liczył od zera
    ...
    data["_swiezak_source"] = fresh_source  # info dla UI
    # Lokalnie wyczyść autopilot_messages też
    if fresh_source == "calculated_recalc":
...
def ew_count_fresh_available(grupa):
    """Licz nieprzeliczone + calculated casy w grupie (ile świeżaków jest).
    
    Zwraca dict: {"unprocessed": N, "calculated": M, "total": N+M}
    """
    try:
        q = (db.collection(col("ew_cases"))
             .where("grupa", "==", grupa)
...
def ew_get_pool_diagnostics(grupa, op_name):
    """Diagnostyka puli casów w grupie — co się dzieje gdy operator dostaje 'Brak wolnych'."""
    skipped_ids = st.session_state.get("ew_skipped_ids", set())
    diag = {
        "grupa": grupa,
        "total_wolne": 0,
...
def ew_complete_case(case_doc_id, result_tag=None, result_pz=None, prompt_name=None, prompt_url=None, prompt_filename=None, prompt_github_link=None, prompt_layer=None, processed_via=None):
    ...
    if prompt_filename: upd["processed_prompt_filename"] = prompt_filename
    if prompt_github_link: upd["processed_prompt_github_link"] = prompt_github_link
    if prompt_layer: upd["processed_prompt_layer"] = prompt_layer
    upd["processed_via"] = processed_via or "manual"
...
with st.sidebar:
    ...
    if not czyste_okno:
        ...
        if show_current_case:
            ...
            is_swiezak = case.get("_is_swiezak", False) or case.get("processed_via") == "swiezak_pending"
            ...
            if is_swiezak:
                ...
                _sw_file = cfg.get("prompt_filename", "?")
                st.success(f"✨ **Świeżak** — będzie liczony promptem: **{_sw_prompt}** (`{_sw_file}`)")
                ...
                if is_reverse:
                    ...
                    with _col_start:
                        if st.button("▶️ Rozpocznij ten case", type="primary", key="start_assigned_case"):
                            _start_clicked = True
                    with _col_swiezak:
                        if st.button("✨ Pobierz świeżaka", type="secondary", key="swiezak_on_assigned", help="Przelicz ten case od nowa świeżym promptem (zamiast nocnej analizy)"):
                            _swiezak_clicked = True
                    
                    if _swiezak_clicked:
                        # Świeżak na obecnie przypisany case — wyczyść autopilot_messages,
                        # oznacz jako świeżak, ten sam case zostanie przeliczony od zera
                        case["_is_swiezak"] = True
                        case["processed_via"] = "swiezak_pending"
                        # Wyczyść w bazie: autopilot_messages żeby nie ładowało starej analizy
                        if case.get("_doc_id"):
                        ...
                        st.toast("✨ Case zostanie przeliczony świeżym promptem", icon="✨")
                        # Symuluj klik Rozpocznij
                        _start_clicked = True
                    ...
                    if _start_clicked:
                        ...
                        if wsad:
                            ...
                            # Reset forum debug log przy rozpoczęciu nowego case — pokazuj tylko ruchy tego zamówienia
                            st.session_state.forum_debug_log = []
                            st.session_state.chat_nrzam = nrzam
                            
                            # --- NIGHT_TAG: podmień stary tag na nocny (TYLKO jeśli nie świeżak) ---
                            _is_swiezak_case = case.get("_is_swiezak") or case.get("processed_via") == "swiezak_pending"
                            _night_tag = case.get("night_tag") if not _is_swiezak_case else None
                            if _night_tag:
            ...
            if st.button("✅ Zakończ → Następny"):
                ...
                if tag:
                    if case.get("_doc_id"):
                        _proc_via = "swiezak" if (case.get("_is_swiezak") or case.get("processed_via") == "swiezak_pending") else "manual"
                        ew_complete_case(case["_doc_id"], result_tag=tag, result_pz=pz, prompt_name=cfg.get("prompt_name"), prompt_url=cfg.get("prompt_url"), prompt_filename=cfg.get("prompt_filename"), prompt_github_link=cfg.get("prompt_github_link"), prompt_layer=cfg.get("prompt_layer"), processed_via=_proc_via)
                ...
                if avail > 0:
                        ...
                        if st.button("📥 Pobierz case", type="primary", key="get_normal_case"):
                            ...
                            if case:
                                ...
                                _msg = (
                                    ...
                                    f"- 🌙 Czekają na uwolnienie nocnych ruchów forum: **{_diag['forum_action']}**\n"
                    ...
                    with _col_fresh:
                        if _fresh_unproc > 0:
                            ...
                            _fresh_label = f"✨ Pobierz świeżaka (przelicz ponownie)"
                            _fresh_help = f"Brak nieprzeliczonych ({_fresh_unproc}). Świeżak weźmie już-przeliczony i policzy go od nowa świeżym promptem ({_fresh_recalc} dostępnych)."
                        else:
                        ...
                        if st.button(_fresh_label, type="secondary", key="get_fresh_case", help=_fresh_help):
                            if _fresh_total == 0:
                                st.warning("Brak nieprzeliczonych i przeliczonych casów do świeżaka. Wszystkie wolne czekają na uwolnienie nocnego forum.")
                            else:
                                ...
                                if case:
                                    ...
                                    if _src == "calculated_recalc":
                                        st.toast("✨ Przeliczam ponownie wcześniej-przeliczonego świeżym promptem", icon="✨")
                                    st.rerun()
                                else:
...
# PARAMETRY STARTOWE
domyslny_operator={get_forum_nick(op_name)}
...
"""
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            if FORUM_ENABLED and ("[FORUM_WRITE|" in ai_text or "[FORUM_READ|" in ai_text):
                                ...
                                forum_result = execute_forum_actions(ai_text, forum_memory=_fm_e2, user_od=cfg.get('role', 'Operatorzy_DE'), ai_user=get_forum_nick(op_name))
                                ...
                                if forum_result["forum_writes"]:
                                    for fw in forum_result["forum_writes"]:
                                        ...
                                        elif fw.get("fallback_needed"):
                                            st.toast(f"⚠️ Forum: błąd — wyślij ręcznie i podaj FORUM_ID", icon="⚠️")
                                            st.session_state["_last_forum_fallback"] = {
                                                "cel": fw.get("cel") or fw.get("fallback_cel"),
                                                "user_do": fw.get("fallback_user_do"),
                                                "tresc_skrot": fw.get("tresc_skrot", ""),
                                            }
                                        else:
    ...
    if prompt := st.chat_input("Odpowiedz AI..."):
        # Detekcja: fallback manualny - operator podaje FORUM_ID wpisu który sam wysłał
        _fid_match = re.search(r'SESJA\s+WYNIK\s+(\S+)\s*[–\-]\s*FORUM_ID:\s*(\d+)', prompt, re.IGNORECASE)
        if _fid_match:
            _fid_nrzam = _fid_match.group(1).strip()
            _fid_id = int(_fid_match.group(2))
            # Pobierz ostatni fallback z sesji (cel + user_do) jeśli był
            _last_fallback = st.session_state.get("_last_forum_fallback")
            if _last_fallback:
                try:
                    save_forum_memory(db, col, _fid_nrzam, _last_fallback.get("cel", "UNKNOWN"), _fid_id, _last_fallback.get("tresc_skrot", ""))
                    st.toast(f"✅ Zapisano FORUM_ID={_fid_id} do pamięci ({_last_fallback.get('cel')})", icon="📝")
                    st.session_state["_last_forum_fallback"] = None
                except Exception as _e_fb:
                    st.warning(f"Nie zapisano do memory: {_e_fb}")
...
if st.session_state.get("forum_debug_log"):
    with st.expander("🔍 Forum Debug Log", expanded=False):
        _layer = cfg.get("prompt_layer", "?")
        _fname = cfg.get("prompt_filename", PROMPT_URL.split('/')[-1] if PROMPT_URL else '?')
        _ghlink = cfg.get("prompt_github_link", "")
        if _ghlink:
            st.caption(f"📄 Prompt: **{PROMPT_NAME}** | Warstwa: **{_layer}** | Plik: `{_fname}` | [GitHub]({_ghlink})")
        else:
            st.caption(f"📄 Prompt: **{PROMPT_NAME}** | Warstwa: **{_layer}** | Plik: `{_fname}`")
